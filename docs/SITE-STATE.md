...
- ~~**No remote is configured**~~ — **since 2026-09-22 `origin` is `https://github.com/jontaylormed/MrFractionWordExpress`**, the only repository to push to; the old `jtaylor-cloud` one is retired. Whether GitHub Pages is switched on for the new repo, and so whether a push publishes the *site*, is **unverified**.
