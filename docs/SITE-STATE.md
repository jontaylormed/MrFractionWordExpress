...
## What shipped on 2026-08-17, and is newest
...
- **Appearance is unverified.** The in-app browser pane does not composite frames. Nothing shipped on 2026-08-17 has been *looked at*. `tools/zz-drive.js` plus headless Edge is the way to close that — its header carries the two flags that cost an hour each.
...
- **No remote is configured**, so publishing is still a manual upload — the route that once flattened `assets/` and `content/` silently.
